...
const likes = await kv.HGET(postTitle, "likes") || await kv.HSET(postTitle, "likes", 0);
````
...
app.get("api/:postTitle/likes", async (req, res) => {
  try {
    ...
    likes = await kv.HGET(postTitle, "likes") || await kv.HSET(postTitle, "likes", 0);
...
<script>
  ...
  const title = viewsP.dataset.title;
